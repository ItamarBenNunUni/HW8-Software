...
public class StarfleetManagerTester {
	...
	public static void main(String[] args) {
		...
		for (Officer officer : sortedOfficers) {
			System.out.println("\t" + officer.getRank() + "\t" + officer.getName() + "\t"
					+ officersToShipsMap.get(officer).getName());
		}

		System.out.println();
		System.out.println("Officer ranks sorted ascendingly by popularity:");
		for (Map.Entry<OfficerRank, Integer> rankCountPair : StarfleetManager
				.getOfficerRanksSortedByPopularity(fleet)) {
	...
	private static String generateName() {
		final String[] nameRepository = new String[] { "Riker", "Kathryn", "Picard", "Archer", "Sisko", "Troi",
				"Crusher", "FitzRoy", "Sparrow", "Nemo", "America", "Data" };
	...
	private static OfficerRank generateRank() {
		final OfficerRank[] ranksRepository = new OfficerRank[] { OfficerRank.Ensign, OfficerRank.Ensign,
				OfficerRank.Commander, OfficerRank.Lieutenant, OfficerRank.Ensign, OfficerRank.LieutenantCommander,
